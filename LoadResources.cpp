...
{
	if (!LoadTextures() || !LoadSprites())
		MessageBox(WINDOW->getWindowHandler(), L"failed to initialize resources", L"Error", MB_ICONERROR);
}
...
bool LoadResources::LoadTextures()
{
	try {
		...
		TEXTURES->Add(1, L"gamedata\\Resources\\simonTEX.png", D3DCOLOR_XRGB(255, 0, 255));
		...
		TEXTURES->Add(2, L"gamedata\\Resources\\Map\\intro\\intro_tilesheet.png", D3DCOLOR_XRGB(1, 1, 1));
...
bool LoadResources::LoadSprites()
{
	...
	if (!ReadSpriteFile(SIMON_DATA_FILEPATH, 1))
		return false;
