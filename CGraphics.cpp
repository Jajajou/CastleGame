...
{
	MessageBox(hWnd, L"failed to initialize Graphics", L"Error", MB_ICONERROR);
	PostQuitMessage(0);
}
...
void CGraphics::BeginGraphics()
{
	...
	{
		MessageBox(0, L"Failed to Start Scene", L"Error", MB_ICONERROR);
		PostQuitMessage(0);
	}
}
...
void CGraphics::EndGraphics()
{
	...
	if (FAILED(hr) || FAILED(hr1))
	{
		MessageBox(0, L"Failed to End Scene ", L"Loi", MB_ICONERROR);
		PostQuitMessage(0);
	}
}
