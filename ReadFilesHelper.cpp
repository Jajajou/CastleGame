...
bool ReadSpriteFile(const char* filePath, int texId, int offset) {
	...
	try {
		...
		infile.open(filePath);
		if (!infile) {
			MessageBox(WINDOW->getWindowHandler(), L"failed to read Sprite data", L"Error", MB_ICONERROR);
			return false;
		}
...
bool ReadObject(const char* filePath)
{
	...
	try {
		...
		infile.open(filePath);
		if (!infile) {
			MessageBox(WINDOW->getWindowHandler(), L"failed to read Object data", L"Error", MB_ICONERROR);
			return false;
		}
